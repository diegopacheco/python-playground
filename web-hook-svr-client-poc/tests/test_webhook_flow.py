...
class WebhookFlowTest(unittest.TestCase):
    def setUp(self):
        ...
        self.server = start(make_handler(HouseStore(), WebhookPublisher(relay_url, SECRET)))
    ...
    def test_every_house_change_reaches_the_listener_as_a_valid_signed_event(self):
    ...
    def test_advancing_a_delivered_house_sends_no_webhook(self):
    ...
    def test_invalid_order_is_rejected_without_sending_a_webhook(self):
        status, _ = self.post("/api/houses", {"model": "castle", "lot": "Lot Z-99", "buyer_alias": "buyer-0099"})
        self.assertEqual(400, status)
        self.assertEqual([], FakeRelay.received)

    def test_relay_down_is_reported_as_502_instead_of_silent_success(self):
        self.relay.shutdown()
