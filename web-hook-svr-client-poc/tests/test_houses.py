...
class HouseStoreTest(unittest.TestCase):
    ...
    def test_new_order_starts_as_ordered_with_catalog_details(self) -> None:
        house = self.store.create("aspen", "Lot A-01", "buyer-0001", "corr-1")
        self.assertEqual("ORDERED", house["status"])
        self.assertEqual(189000, house["model_details"]["price_usd"])

    def test_every_stage_keeps_the_correlation_id_of_the_order(self) -> None:
        house = self.store.create("cedar", "Lot C-03", "buyer-0003", "journey-42")
        for _ in STAGES[1:]:
            _, house = self.store.advance(house["id"])
            self.assertEqual("journey-42", house["correlation_id"])

    def test_house_moves_through_every_construction_stage_in_order(self) -> None:
        house = self.store.create("birch", "Lot B-02", "buyer-0002", "corr-1")
        seen = []
        for _ in STAGES[1:]:
    ...
    def test_a_delivered_house_cannot_move_again(self) -> None:
        house = self.store.create("dune", "Lot D-04", "buyer-0004", "corr-1")
        for _ in STAGES[1:]:
    ...
    def test_unknown_model_is_rejected_so_no_webhook_carries_an_unpriced_house(self) -> None:
        with self.assertRaises(ValueError):
            self.store.create("castle", "Lot Z-99", "buyer-0099", "corr-1")

    def test_lot_and_buyer_alias_are_required(self) -> None:
        with self.assertRaises(ValueError):
            self.store.create("aspen", "", "buyer-0001", "corr-1")
