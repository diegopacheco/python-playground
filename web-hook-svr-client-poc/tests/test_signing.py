...
class SigningTest(unittest.TestCase):
    def test_signature_from_the_server_is_accepted(self):
        signature = sign(SECRET, "1700000000", PAYLOAD)
        self.assertTrue(verify(SECRET, "1700000000", PAYLOAD, signature))

    def test_relay_reserializing_the_body_with_other_key_order_still_verifies(self):
    ...
    def test_anyone_posting_to_the_public_relay_without_the_secret_is_rejected(self):
    ...
    def test_replaying_a_signature_with_another_timestamp_is_rejected(self):
        signature = sign(SECRET, "1700000000", PAYLOAD)
