...
ALLOWED_HEADERS = ("x-webhook-event", "x-webhook-timestamp", "x-webhook-signature")
...
def to_event(message: dict, secret: str) -> dict | None:
    ...
    if not isinstance(body, dict):
        return None
    headers = {name: message.get(name) for name in ALLOWED_HEADERS}
    return {
        "id": str(body.get("id") or uuid.uuid4()),
        "type": str(headers["x-webhook-event"] or "unknown"),
        "signature_valid": verify(secret, headers["x-webhook-timestamp"], body, headers["x-webhook-signature"]),
        "received_at": now_iso(),
        "payload": body,
    }
...
class RelaySubscriber:
    def __init__(self, relay_url: str, secret: str, store: EventStore):
